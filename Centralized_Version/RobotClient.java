...
/**
 * A very naive implementation of a computer controlled {@link LocalClient}.
 * Basically it stumbles about and shoots.
 * 
...
public class RobotClient extends LocalClient implements Runnable {
  ...
  /**
   * Random number generator so that the robot can be "non-deterministic".
   */
   ...
   * 
   * @param name
   *          The name of this {@link RobotClient}.
   */
  ...
  /**
   * Override the abstract {@link Client}'s registerMaze method so that we know
   * when to start control thread.
   * 
   * @param maze
   *          The {@link Maze} that we are begin registered with.
   */
  ...
  /**
   * Override the abstract {@link Client}'s unregisterMaze method so we know
   * when to stop the control thread.
   */
  ...
  public void run() {
    ...
    Mazewar
        .consolePrintLn("Robot client \"" + this.getName() + "\" activated.");
